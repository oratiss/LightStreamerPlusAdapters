#!/bin/sh
# Do not remove this line. File tag: linux_launch-APV-7.2.0.
...
if [ "$1" = "restart" ] ; then
   ...
   write_pid
   exec "$JAVA" $JAVA_OPTS "$KERNEL_LIB_PATH" "$INTERNAL_LIB_PATH" "$LOGGING_LIB_PATH" "$MPN_LIB_PATH" -cp "$cpath" $class "$arg1"
   ...
   write_pid
   exec "$JAVA" $JAVA_OPTS "$KERNEL_LIB_PATH" "$INTERNAL_LIB_PATH" "$LOGGING_LIB_PATH" "$MPN_LIB_PATH" -cp "$cpath" $class "$arg1" >> "$LS_HOME/logs/LS.out" 2>&1 &
